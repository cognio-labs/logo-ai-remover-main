...
import {
  CheckCircle2, Clipboard, Download, Image as ImageIcon, LoaderCircle, Pause, Play,
  RefreshCw, Sparkles, Square, UploadCloud, WandSparkles, X,
} from "lucide-react";
import { fetchFile } from "@ffmpeg/util";
import { toast } from "sonner";
import { PinkButton } from "./PinkButton";
import {
  commandLine, delogoFilter, even, getLocalFfmpeg, outputName, paddedMask,
  terminateLocalFfmpeg, type MaskRect,
...
export function LocalMediaCleaner({
...
}: { demoUrl?: string; demoName?: string }) {
  ...
  const dragRef = useRef<null | {
    mode: "move" | "resize"; clientX: number; clientY: number; mask: MaskRect;
  }>(null);
  ...
  const completeFrames = status === "done" ? totalFrames : Math.min(totalFrames, Math.round(progress * totalFrames));
  ...
  const clearResults = () => {
    ...
    setPreviewUrl(null); setOutputUrl(null); setJpgUrl(null);
    setError(""); setProgress(0); setDownloadQueued(false);
  };
  const reset = () => {
    runIdRef.current += 1;
    ...
    setSourceUrl(null); setFile(null); setStatus("idle"); setLogs([]);
  ...
  const loadFile = async (nextFile: File) => {
    ...
    if (nextFile.size > 500 * 1024 * 1024) {
      toast.warning("This file is over 500 MB. Browser memory may be limited; the CLI guide is safer.");
    }
    if (sourceUrl) URL.revokeObjectURL(sourceUrl);
    clearResults();
    const url = URL.createObjectURL(nextFile);
    setFile(nextFile); setSourceUrl(url); setKind(nextKind); setStatus("idle"); setLogs([]);
    try {
      const metadata = await new Promise<{ width: number; height: number; duration: number }>((resolve, reject) => {
        if (nextKind === "image") {
          ...
          video.onerror = () => reject(new Error("The video could not be decoded by this browser."));
          video.src = url;
        }
      });
  ...
  useEffect(() => {
    const paste = (event: ClipboardEvent) => {
      ...
      if (pasted) { event.preventDefault(); void loadFile(pasted); }
    };
  ...
  const autoDetect = async () => {
    ...
    try {
      if (kind === "image") {
        ...
        const video = document.createElement("video");
        video.muted = true; video.preload = "auto"; video.src = sourceUrl;
      ...
      for (let y = 0; y <= sampleHeight - patchSize; y += step) {
        for (let x = 0; x <= sampleWidth - patchSize; x += step) {
          let temporal = 0; let contrast = 0; let points = 0;
          for (let py = y; py < y + patchSize; py += 2) {
            for (let px = x; px < x + patchSize; px += 2) {
              const offset = (py * sampleWidth + px) * 4;
              const values = frames.map((frame) => (frame[offset] + frame[offset + 1] + frame[offset + 2]) / 3);
              const mean = values.reduce((sum, value) => sum + value, 0) / values.length;
              temporal += values.reduce((sum, value) => sum + Math.abs(value - mean), 0) / values.length;
              if (px + 2 < sampleWidth) {
                contrast += Math.abs(mean - (frames[0][offset + 8] + frames[0][offset + 9] + frames[0][offset + 10]) / 3);
              }
              points += 1;
            }
          }
          const edgeDistance = Math.min(x, y, sampleWidth - x - patchSize, sampleHeight - y - patchSize);
  ...
  const runCommand = async (
    ffmpeg: Awaited<ReturnType<typeof getLocalFfmpeg>>,
    args: string[],
  ) => {
  ...
  const startCleanup = async () => {
    ...
    setStatus("loading"); setLogs([]); setError(""); setProgress(0);
    let ffmpeg: Awaited<ReturnType<typeof getLocalFfmpeg>> | null = null;
    let inputName = "";
    const made: string[] = [];
    const logHandler = ({ message }: { message: string }) => addLog(message);
    ...
    try {
      ...
      ffmpeg.on("log", logHandler); ffmpeg.on("progress", progressHandler);
      ...
      const previewMask = {
        x: even(finalMask.x * previewScale), y: even(finalMask.y * previewScale),
        w: even(finalMask.w * previewScale, 2), h: even(finalMask.h * previewScale, 2),
      };
      ...
      if (kind === "video") {
        ...
        await runCommand(ffmpeg, [
          "-y", "-i", inputName,
          "-vf", `scale=-2:${previewHeight},${delogoFilter(previewMask)}`,
          "-c:v", "libx264", "-crf", "23", "-preset", "veryfast",
          "-pix_fmt", "yuv420p", "-an", "-movflags", "+faststart", preview,
        ]);
        made.push(preview);
        const previewResult = objectUrl(await ffmpeg.readFile(preview), "video/mp4");
        if (runId !== runIdRef.current) { URL.revokeObjectURL(previewResult); return; }
        setPreviewUrl(previewResult); setStatus("preview"); setProgress(0);
        addLog("Fast preview ready. Starting full-resolution export.");

        const output = "output.mp4";
        const baseArgs = [
          "-y", "-i", inputName, "-vf", delogoFilter(finalMask),
          "-c:v", "libx264", "-crf", "14", "-preset", "medium", "-pix_fmt", "yuv420p",
        ];
        setStatus("exporting");
        try {
          await runCommand(ffmpeg, [...baseArgs, "-c:a", "copy", "-movflags", "+faststart", output]);
        } catch (copyProblem) {
          addLog(`Audio copy failed; retrying AAC: ${copyProblem instanceof Error ? copyProblem.message : String(copyProblem)}`);
          try { await ffmpeg.deleteFile(output); } catch {}
          await runCommand(ffmpeg, [...baseArgs, "-c:a", "aac", "-b:a", "192k", "-movflags", "+faststart", output]);
        }
        made.push(output);
        const fullResult = objectUrl(await ffmpeg.readFile(output), "video/mp4");
        if (runId !== runIdRef.current) { URL.revokeObjectURL(fullResult); return; }
        ...
        await runCommand(ffmpeg, ["-y", "-i", inputName, "-vf", delogoFilter(finalMask), "-frames:v", "1", output]);
        made.push(output);
        setOutputUrl(objectUrl(await ffmpeg.readFile(output), "image/png"));
        if (isJpg) {
          const jpg = "output.jpg";
          await runCommand(ffmpeg, ["-y", "-i", inputName, "-vf", delogoFilter(finalMask), "-frames:v", "1", "-q:v", "2", jpg]);
          made.push(jpg);
      ...
      setError(message.includes("SharedArrayBuffer")
        ? "The local engine could not start. Try current Chrome or Edge."
        : `Cleanup failed: ${message}`);
      setStatus("error");
    } finally {
      if (ffmpeg) {
        ffmpeg.off("log", logHandler); ffmpeg.off("progress", progressHandler);
        for (const name of [inputName, ...made]) {
          if (name) { try { await ffmpeg.deleteFile(name); } catch {} }
        }
      }
    }
  };
  ...
  const cancel = () => {
    runIdRef.current += 1;
    terminateLocalFfmpeg();
    setStatus(previewUrl ? "preview" : "idle"); setProgress(0);
  ...
  const togglePlayback = async () => {
    ...
    if (!original) return;
    if (playing) {
      original.pause(); processed?.pause(); setPlaying(false);
    } else {
  ...
  const cliArgs = file
    ? kind === "video"
      ? ["-i", file.name, "-vf", delogoFilter(finalMask), "-c:v", "libx264", "-crf", "14", "-preset", "medium", "-pix_fmt", "yuv420p", "-c:a", "copy", "-movflags", "+faststart", outputName(file.name, "mp4")]
      : ["-i", file.name, "-vf", delogoFilter(finalMask), "-frames:v", "1", outputName(file.name, "png")]
    : [];
  ...
  if (!file || !sourceUrl) {
    return (
      ...
      >
        ...
        <h3 className="mt-5 text-2xl font-semibold tracking-tight text-gray-950">Drop your video or image here</h3>
        ...
        <div className="mt-6 flex flex-wrap justify-center gap-3">
          <PinkButton type="button" onClick={(event) => { event.stopPropagation(); inputRef.current?.click(); }}>
            <UploadCloud className="size-4" /> Upload media
          </PinkButton>
          <PinkButton type="button" variant="outline" onClick={(event) => { event.stopPropagation(); void loadDemo(); }}>
            <Sparkles className="size-4" /> Try demo sample
          </PinkButton>
        </div>
        <p className="mt-6 text-xs font-normal text-gray-400">Your file stays in this browser and is never uploaded.</p>
        <input ref={inputRef} className="hidden" type="file" accept={ACCEPT} onChange={(event) => {
    ...
    <div className="rounded-3xl border border-[#FCE7EC] bg-white p-3 text-left shadow-[0_15px_45px_-10px_rgba(225,29,72,0.14)] sm:p-5">
      <div className="flex flex-wrap items-center justify-between gap-3 border-b border-gray-100 pb-4">
        <div className="min-w-0">
          ...
          <p className="mt-0.5 text-xs font-normal text-gray-500">{size.width} × {size.height}{kind === "video" ? ` • ${formatTime(duration)}` : ""} • processed locally</p>
        </div>
        <div className="flex flex-wrap items-center gap-2">
          <button type="button" onClick={() => inputRef.current?.click()} className="inline-flex items-center gap-1.5 rounded-xl px-3 py-2 text-xs font-medium text-gray-600 hover:bg-gray-50">
            <RefreshCw className="size-3.5" /> Choose another
          </button>
          <button type="button" onClick={reset} className="inline-flex items-center gap-1.5 rounded-xl px-3 py-2 text-xs font-medium text-gray-600 hover:bg-gray-50">
      ...
      <div className="mt-4 grid grid-cols-1 overflow-hidden rounded-2xl border border-gray-200 md:grid-cols-2 md:divide-x">
        <div className="border-b border-gray-200 p-3 md:border-b-0">
          <div className="mb-2 flex items-center justify-between">
            <span className="rounded-lg bg-gray-950 px-2.5 py-1 text-[11px] font-semibold text-white">Original</span>
            <span className="text-[11px] font-normal text-gray-400">Place box over the full logo</span>
          </div>
          <div ref={stageRef} className="relative aspect-video w-full touch-none overflow-hidden rounded-xl bg-black"
            onPointerMove={moveBox}
            onPointerUp={() => { dragRef.current = null; }}
            onPointerCancel={() => { dragRef.current = null; }}>
            {kind === "video" ? (
              <video ref={originalVideoRef} src={sourceUrl} playsInline
                onLoadedMetadata={(event) => setDuration(event.currentTarget.duration)}
                onTimeUpdate={(event) => {
                  const time = event.currentTarget.currentTime;
                  setCurrentTime(time);
                  const processed = processedVideoRef.current;
                  if (processed && Math.abs(processed.currentTime - time) > 0.1) processed.currentTime = time;
                }}
                onEnded={() => setPlaying(false)}
                className="size-full object-contain" />
            ) : (
              <img ref={originalImageRef} src={sourceUrl} alt="Original upload" className="size-full object-contain" />
            )}
            <div role="group" aria-label="Logo removal area" style={boxStyle}
              ...
              <span className="absolute -top-6 left-0 rounded bg-[#E11D48] px-1.5 py-0.5 text-[9px] font-semibold text-white">Logo area</span>
              <button type="button" aria-label="Resize logo area"
        ...
        <div className="p-3">
          <div className="mb-2 flex items-center justify-between">
            <span className="rounded-lg bg-gray-950 px-2.5 py-1 text-[11px] font-semibold text-white">Processed</span>
            {status === "done" && <span className="inline-flex items-center gap-1 text-[11px] font-medium text-emerald-600"><CheckCircle2 className="size-3.5" /> Full quality ready</span>}
          </div>
          <div className="relative aspect-video w-full overflow-hidden rounded-xl bg-black">
            {resultUrl ? (
              kind === "video"
                ? <video ref={processedVideoRef} src={resultUrl} muted playsInline className="size-full object-contain" />
              ...
              <div className="flex size-full flex-col items-center justify-center px-6 text-center text-gray-400">
                {busy ? <LoaderCircle className="mb-2 size-7 animate-spin text-[#E11D48]" /> : <WandSparkles className="mb-2 size-7 text-[#FF4FA3]" />}
                <p className="text-xs font-normal">{busy ? "Creating a real local preview…" : "Ready to process"}</p>
              </div>
            )}
          </div>
        </div>
      ...
      {kind === "video" && (
        <div className="mt-3 flex items-center gap-3 rounded-xl border border-gray-200 px-3 py-2.5">
          ...
          <input aria-label="Video position" type="range" min={0} max={duration || 0} step="0.01"
            value={Math.min(currentTime, duration || 0)}
            onChange={(event) => seek(Number(event.target.value))}
            className="h-1.5 min-w-0 flex-1 accent-[#E11D48]" />
          <span className="w-24 text-right font-mono text-[11px] text-gray-500">{formatTime(currentTime)} / {formatTime(duration)}</span>
        </div>
      )}
      ...
      <div className="mt-4 grid gap-4 lg:grid-cols-[1fr_auto] lg:items-end">
        <div>
          <div className="flex items-center justify-between gap-4">
            <label htmlFor="mask-padding" className="text-xs font-medium text-gray-700">Padding: {padding}px</label>
          ...
          <input id="mask-padding" type="range" min={0} max={40} value={padding}
            onChange={(event) => setPadding(Number(event.target.value))}
            disabled={busy} className="mt-2 h-1.5 w-full accent-[#E11D48]" />
          <p className="mt-1 text-[11px] font-normal text-gray-400">Source pixels: x {finalMask.x}, y {finalMask.y}, w {finalMask.w}, h {finalMask.h}</p>
        </div>
        <div className="flex flex-wrap gap-2">
          <button type="button" onClick={() => setCliOpen(true)}
      ...
      {(["loading", "exporting", "preview", "done"] as Status[]).includes(status) && (
        <div className="mt-4 rounded-xl border border-gray-200 bg-gray-50 p-3">
          <div className="mb-2 flex items-center justify-between gap-4 text-xs">
            ...
            <span className="font-mono text-[11px] text-gray-500">{Math.round(progress * 100)}%</span>
          </div>
          <div className="h-2 overflow-hidden rounded-full bg-gray-200">
            <div className="h-full rounded-full bg-gradient-to-r from-[#E11D48] to-[#FF4FA3] transition-[width]"
      ...
      {error && (
        <div className="mt-4 rounded-xl border border-red-200 bg-red-50 p-3 text-sm text-red-800">
          <p>{error}</p>
          <button type="button" onClick={() => void startCleanup()}
            className="mt-2 text-xs font-semibold underline underline-offset-2">Retry</button>
        </div>
      )}

      <div className="mt-4 flex flex-wrap items-center gap-2">
        <button type="button" aria-disabled={status !== "done"}
        ...
        {kind === "image" && isJpg && (
          <button type="button" aria-disabled={status !== "done"} onClick={() => requestDownload("jpg")}
        ...
        {downloadQueued && status !== "done" && <span className="text-xs font-medium text-gray-500">Preparing HD file…</span>}
      </div>

      <details className="mt-4 rounded-xl border border-gray-200 bg-white">
        <summary className="cursor-pointer px-3 py-2 text-xs font-medium text-gray-600">Show log</summary>
          ...
          <div className="w-full max-w-2xl rounded-3xl bg-white p-6 shadow-2xl"
            ...
            <div className="flex items-start justify-between gap-4">
              <div>
                ...
                <p className="mt-1 text-xs text-gray-500">Use this for very large files. Values match the current box and padding.</p>
              </div>
              <button type="button" onClick={() => setCliOpen(false)}
                className="rounded-full p-2 text-gray-500 hover:bg-gray-100"><X className="size-4" /></button>
            </div>
            <pre className="mt-4 overflow-auto rounded-xl bg-gray-950 p-4 text-xs leading-relaxed text-gray-100">{commandLine(cliArgs)}</pre>
            <div className="mt-4 flex justify-end">
              <PinkButton type="button" onClick={() => {
                void navigator.clipboard.writeText(commandLine(cliArgs));
                toast.success("Command copied.");
              }}><Clipboard className="size-4" /> Copy command</PinkButton>
            </div>
          </div>
        </div>
      )}
